package org.cleverframe.filemanager;

import org.cleverframe.common.IBeanNames;

/**
 * 作者：LiZW <br/>
 * 创建时间：2016/11/17 22:11 <br/>
 */
public class FilemanagerBeanNames implements IBeanNames {
    // -------------------------------------------------------------------------------------------//
    // Dao
    // -------------------------------------------------------------------------------------------//
    public static final String FileInfoDao = "filemanager_FileInfoDao";


    // -------------------------------------------------------------------------------------------//
    // Service
    // -------------------------------------------------------------------------------------------//
    public static final String FileManagerService = "filemanager_FileManagerService";
    public static final String LocalStorageService = "filemanager_LocalStorageService";
    public static final String FtpStorageService = "filemanager_FtpStorageService";


    // -------------------------------------------------------------------------------------------//
    // Other
    // -------------------------------------------------------------------------------------------//

}
