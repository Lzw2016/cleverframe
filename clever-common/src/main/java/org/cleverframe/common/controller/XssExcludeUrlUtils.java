...
public class XssExcludeUrlUtils {
    ...
    static {
        ...
        if (config == null) {
            ...
            XSS_EXCLUDE_URL_SAVE_FILE = config.getConfig(FilemanagerConfigNames.FILE_STORAGE_PATH) + File.separator + "XssExcludeUrlSaveFile.txt";
            ...
            if (!file.exists()) {
                ...
                if (!flag) {
                    String tmp = "### 创建文件失败:[" + XSS_EXCLUDE_URL_SAVE_FILE + "]. 请配置cleverframe.properties文件的值: [" + FilemanagerConfigNames.FILE_STORAGE_PATH + "]";
    ...
    public static int loadXSSExcludeUrl() {
        if (StringUtils.isBlank(XSS_EXCLUDE_URL_SAVE_FILE)) {
            logger.error("请配置cleverframe.properties文件的值: [" + FilemanagerConfigNames.FILE_STORAGE_PATH + "]");
            return 0;
        }
